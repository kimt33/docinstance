...
def test_make_numpy_docstring():
    ...
    assert (
        test.make_numpy_docstring(26, 0, 4)
        == "var_name : {str, int,\n            bool}\n    hello\n"
    )
    assert (
        test.make_numpy_docstring(27, 1, 2)
        == "  var_name : {str, int,\n              bool}\n    hello\n"
    )
    assert (
        test.make_numpy_docstring(17, 0, 4)
        == "var_name : {str,\n            int,\n            bool}\n    hello\n"
    )
    ...
    assert (
        test.make_numpy_docstring(27, 0, 4)
...
def test_make_numpy_docstring_signature():
    ...
    assert (
        test.make_numpy_docstring_signature(36, 0, 4)
        == "var_name(a, b, c) : {str, int, bool}\n    hello\n"
    )
    assert (
        test.make_numpy_docstring_signature(26, 0, 4)
        == "var_name(a, b, c) : {str,\n                     int,\n                     bool}\n"
        "    hello\n"
    )
...
def test_make_google_docstring():
    ...
    assert test.make_google_docstring(23, 0, 4) == (
        "var_name (:obj:`str`,\n"
        "          :obj:`int`,\n"
        "          :obj:`bool`):\n"
        "    hello\n"
    )
    assert test.make_google_docstring(26, 1, 2) == (
        "  var_name (:obj:`str`,\n"
        "            :obj:`int`,\n"
        "            :obj:`bool`):\n"
        "    hello\n"
    ...
    assert test.make_google_docstring(26, 0, 2) == (
        "var_name (:obj:`str`,\n"
        "          :obj:`int`,\n"
        "          :obj:`bool`):\n"
        "  description 1\n"
...
def test_make_rst_docstring():
    ...
    assert test.make_rst_docstring(25, 0, 4) == (
        ":param var_name: hello my\n" "    name is\n" "    Example 2.\n"
    )
    # name + type
    test = DocDescription("var_name", types=["str", int])
    assert test.make_rst_docstring(20, 0, 2) == (
        ":param var_name:\n" ":type var_name: str,\n" "  :obj:`int`\n"
    )
    # name + desc + type
    test = DocDescription("var_name", types=["str", int], descs=["Example 1.", "Example 2."])
    assert test.make_rst_docstring(27, 0, 4) == (
