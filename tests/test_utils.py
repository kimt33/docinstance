...
def test_wrap():
    ...
    # white spaces
    assert docinstance.utils.wrap("    hello", width=9, indent_level=0, tabsize=4) == ["    hello"]
    ...
    assert docinstance.utils.wrap(
        "    hello   my   name   is    ", width=8, indent_level=0, tabsize=4
    ) == ["hello", "my", "name", "is"]
    assert docinstance.utils.wrap(
        "\n\n    hello   my   name   is\n\n", width=5, indent_level=0, tabsize=4
    ) == ["", "", "hello", "my", "name", "is", "", ""]
    assert docinstance.utils.wrap(
        "\n\n    hello\n   my   name   is\n\n", width=5, indent_level=0, tabsize=4
    ) == ["", "", "hello", "   my", "name", "is", "", ""]
    # example
    assert docinstance.utils.wrap(
        "  Text that will be wrapped into different lines such that each"
        " line is indented and is less than the given length.",
        width=30,
    ) == [
    ...
    # subsequent indent
    assert docinstance.utils.wrap(
        "a b c d e", width=4, indent_level=0, tabsize=4, subsequent_indent="xxx"
    ) == ["a b", "xxxc", "xxxd", "xxxe"]
    assert docinstance.utils.wrap(
        "a b c d e", width=4, indent_level=1, tabsize=1, subsequent_indent="xx"
    ) == [" a b", " xxc", " xxd", " xxe"]
    with pytest.raises(ValueError):
        docinstance.utils.wrap(
            "a b c d e", width=4, indent_level=0, tabsize=4, subsequent_indent="xxxx"
        )
...
def test_wrap_indent_subsequent():
    ...
    assert docinstance.utils.wrap_indent_subsequent(
        "a b c d e", width=4, indent_level=1, tabsize=3
    ) == ["a b", "   c", "   d", "   e"]
    with pytest.raises(ValueError):
