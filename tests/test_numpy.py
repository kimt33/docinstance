...
def test_compare_docinstances():
        ...
        [
            ...
            DocSection(
                ...
                [
                    ...
                    DocDescription("name2", "(c, d)", int, ["desc3", DocEquation("desc4")]),
                ],
            ),
        ]
    )
    ...
    assert Docstring(DocSection("a", DocDescription("x", "y", "z", "k"))) != Docstring(
        DocSection("a", DocDescription("1", "y", "z", "k"))
    )
    assert Docstring(DocSection("a", DocDescription("x", "y", "z", "k"))) != Docstring(
        DocSection("a", DocDescription("x", "1", "z", "k"))
    )
    assert Docstring(DocSection("a", DocDescription("x", "y", "z", "k"))) != Docstring(
        DocSection("a", DocDescription("x", "y", "1", "k"))
    )
    assert Docstring(DocSection("a", DocDescription("x", "y", "z", "k"))) != Docstring(
        DocSection("a", DocDescription("x", "y", "z", "1"))
    )
    assert Docstring(DocSection("a", DocDescription("x", "y", "z", DocEquation("k")))) != Docstring(
        DocSection("a", DocDescription("x", "y", "z", DocEquation("1")))
    )
...
def test_parse_numpy():
    ...
    assert parse_numpy(docstring) == Docstring(
        [Summary("summary"), ExtendedSummary(["block1", "block2"])]
    )
    docstring = "\nsummary\n\nblock1\n\nblock2"
    assert parse_numpy(docstring) == Docstring(
        [Summary("summary"), ExtendedSummary(["block1", "block2"])]
    ...
    docstring = "\nsummary\n\n\n\nblock2\n\n"
    assert parse_numpy(docstring) == Docstring([Summary("summary"), ExtendedSummary(["block2"])])
    # FIXME: is this a bug?
    docstring = "\n\nsummary\n\nblock1\n\nblock2"
    assert parse_numpy(docstring) == Docstring(
        [Summary("summary"), ExtendedSummary(["block1", "block2"])]
    ...
    docstring = "summary\n\nblock1\n\nblock2\n\nheader\n------\nstuff"
    assert parse_numpy(docstring) == Docstring(
        [Summary("summary"), ExtendedSummary(["block1", "block2"]), DocSection("header", "stuff")]
    )
    ...
    for header in [
        ...
        assert parse_numpy(docstring) == Docstring(
            [
                ...
                DocSection(header, DocDescription("abc", descs=["description1.", "description2."])),
            ]
        )
        ...
        assert parse_numpy(docstring) == Docstring(
            [
                ...
                DocSection(
                    header,
                    DocDescription("abc", types="str", descs=["description1.", "description2."]),
                ),
            ]
        )
        docstring = (
        ...
        assert parse_numpy(docstring) == Docstring(
            [
                ...
                DocSection(
                    header,
                    DocDescription(
                        "abc", types=["str", "int"], descs=["description1.", "description2."]
                    ),
                ),
            ]
        )
        ...
        assert parse_numpy(docstring) == Docstring(
            [
                ...
                DocSection(
                    header,
                    DocDescription(
                        "abc", signature="(x, y)", descs=["description1.", "description2."]
                    ),
                ),
            ]
        )
        ...
        assert parse_numpy(docstring) == Docstring(
            [
                Summary("summary"),
                DocSection(
                    header, [DocDescription("abc", types="str"), DocDescription("def", types="int")]
                ),
            ]
        )
...
def test_parse_numpy_raw():
    ...
    docstring = '"""summary\n\nextended"""'
    assert parse_numpy(docstring, contains_quotes=True) == Docstring(
        [Summary("summary"), ExtendedSummary("extended")]
    )
    docstring = 'r"""summary\n\nextended"""'
...
def test_parse_numpy_self():
    ...
    # summary
    assert (
        parse_numpy(docstring, contains_quotes=False).sections[0].contents[0]
    ...
    # parameters
    assert parse_numpy(docstring, contains_quotes=False).sections[2].header == "parameters"
    assert parse_numpy(docstring, contains_quotes=False).sections[2].contents[0].name == "docstring"
    assert parse_numpy(docstring, contains_quotes=False).sections[2].contents[0].types == ["str"]
    assert parse_numpy(docstring, contains_quotes=False).sections[2].contents[0].descs == [
        "Numpy docstring."
    ]
    assert (
        parse_numpy(docstring, contains_quotes=False).sections[2].contents[1].name
        == "contains_quotes"
    )
    assert parse_numpy(docstring, contains_quotes=False).sections[2].contents[1].types == ["bool"]
    assert parse_numpy(docstring, contains_quotes=False).sections[2].contents[1].descs == [
        r"True if docstring contains \"\"\" or \'\'\'."
    ]
    # returns
    assert parse_numpy(docstring, contains_quotes=False).sections[3].header == "returns"
    assert parse_numpy(docstring, contains_quotes=False).sections[3].contents[0].name == "docstring"
    ...
    # raises
    assert parse_numpy(docstring, contains_quotes=False).sections[4].header == "raises"
    assert (
        parse_numpy(docstring, contains_quotes=False).sections[4].contents[0].name == "ValueError"
    )
    ...
    assert (
        parse_numpy(docstring, contains_quotes=False).sections[4].contents[1].name
        == "NotImplementedError"
    )
...
def test_parse_numpy_equations():
    ...
    docstring = "summary\n\n.. math::\n\n    \\frac{1}{2}"
    assert parse_numpy(docstring) == Docstring(
        [Summary("summary"), ExtendedSummary(DocEquation("\\frac{1}{2}"))]
    )
    docstring = "summary\n\n" ".. math::\n\n" "    x &= 2\\\\\n" "    &= y\\\\\n"
    assert parse_numpy(docstring) == Docstring(
        [Summary("summary"), ExtendedSummary(DocEquation("x &= 2\\\\\n&= y\\\\"))]
    )
    docstring = (
        "summary\n\n"
        ".. math::\n\n"
    ...
    assert parse_numpy(docstring) == Docstring(
        [
            Summary("summary"),
            Parameters(DocDescription("a", types="float", descs=DocEquation("\\frac{1}{2}"))),
        ]
    )
    # multi line equation
    docstring = (
    ...
    assert parse_numpy(docstring) == Docstring(
        [
            ...
            Parameters(
                DocDescription(
                    "a", types="float", descs=DocEquation("\\frac{1}{2}\\\\\n" "\\frac{1}{3}\n")
                )
            ),
        ]
    )
        ...
        [
            ...
            Parameters(
                DocDescription(
                    "a",
                    types="float",
                    descs=[DocEquation("\\frac{1}{2}"), DocEquation("\\frac{1}{3}")],
                )
            ),
        ]
    )
